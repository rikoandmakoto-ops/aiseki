...
--
--  ⚠ このファイルの purchase_points(int, text) は既に廃止されています。
--    認証済みユーザーなら誰でも呼べる＝支払わずにポイントを増やせるため、
--    supabase/migration_stripe_payments.sql で service_role 専用に
--    作り直しました。このファイルを再実行した場合は、必ずそのあとに
--    migration_stripe_payments.sql を実行してください。
--
