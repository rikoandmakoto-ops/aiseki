...
最終更新: 2026-08-28（**新しい決済・マッチングフロー**を実装した §18。ホストはグループを作ってから卓を立てる・完全無料。ゲストは1名から参加可で料金は常に7,600pt。写真はマッチまで薄モザイク。**DB は適用済み・本番デプロイは未実施**）
...
- ⛔ **本番へのデプロイ**（`vercel deploy --prebuilt --prod`。手順は §2 / §15）。
  **DB は適用済みなので、デプロイまでの間は画面と DB の仕様がずれている。**
- ⛔ 実機（スマートフォン）での確認。
- ⛔ 簡易登録の**確認メール → ログイン → 招待の引き受け**を実メールで通した確認
  （ローカルではコードの控えと引き受けのコードまでは通してある）。
- ⛔ 既存ユーザーの写真には `avatar_blur_url` が無い。
  プロフィールを開いて写真を上げ直すまで、一覧・詳細では絵文字のままになる
  （壊れはしない。`HostThumb` がぼかしの有無で出し分けている）。
