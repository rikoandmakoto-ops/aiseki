...
同日 **インフルエンサー営業DMの管理画面 `/admin/dm` を追加** §29、
**その入口を「運営メール ＋ 管理者パスワード」の2段に締め直して本番へデプロイ済み** §30。
...
- ✅ **2026-08-30 に本番へデプロイ済み**（§30 の2段ゲートと同じ回。`dpl_ACJdc41Wstxh5ocn9MnNbaU6qu7y`）。
- ⛔ 実際の営業リストはまだ0件（`dm_targets`）。ひな形だけ既定の1件が入っている。

---

## 30. 運営画面の締め直し — メール ＋ 管理者パスワードの2段（2026-08-30）

`/admin/dm`（インフルエンサー営業・§29）が **ログインさえしていれば画面だけは開けた**。
API は `requireAdmin()` が 403 を返していたので中身は1件も出ていなかったが、
「運営用の画面がある」ことと、タブ構成・注意書きまでは誰でも読めていた。
ここを **サーバ判定の2段**にした。

| 段 | 何を見るか | 出典（唯一） |
|---|---|---|
| 1段目 | ログイン中のメールアドレス | `api/_lib.js` の `ADMIN_EMAILS`（＝ `theoffzaki@gmail.com`） |
| 2段目 | 管理者パスワード | 環境変数 **`ADMIN_PASSWORD`**（Vercel / `.env`） |

### 30-a. 入っているもの

```
api/_lib.js            requireAdminUnlocked() / issueAdminUnlock() / adminUnlockValid()
                       adminPasswordMatches()（ダイジェスト同士の timing-safe 比較）
api/admin/gate.js      GET  … 運営か・合言葉を通しているか
                       POST … 合言葉の照合と証明の発行（連続失敗5回で10分ロック）
api/dm/*.js            requireAdmin → **requireAdminUnlocked** に差し替え（4ファイル10箇所）
src/lib/adminApi.js    証明の保管（sessionStorage）と x-admin-unlock ヘッダ / fetchAdminGate / unlockAdmin
src/screens/AdminDMScreen.jsx  入口の状態機械（checking → locked → ready）と合言葉の入力欄
src/screens/AdminScreen.jsx    /admin/dm への導線は、サーバが運営と認めたときだけ出す
```

### 30-b. 挙動

- **運営以外が `/admin/dm` を開く** … 画面を描かずに `location.replace("/")` でトップへ。
  判定は `GET /api/admin/gate` の 403（画面側でメールアドレスを見ていない）。
- **運営が開く** … 管理者パスワードの入力欄。通ると 8時間ぶんの証明が返り、
  `sessionStorage`（`aiseki.admin.unlock`）に入る。**タブを閉じれば入力し直し。**
- **証明が切れた / 無い** … `/api/dm/*` は **423** を返す。画面は入口の判定に戻り、
  もう一度パスワードを聞く（`isDenied()` が 423 も拾うので、既存の呼び出し側は無改修）。
- **`/admin`（通報・お問い合わせ）は1段のまま。** 合言葉を足していない。

### 30-c. 触るときの注意

- 🚨 **`ADMIN_PASSWORD` が未設定なら通さない（fail closed）。** `/api/dm/*` は 503 になる。
  「未設定なら合言葉なしで通す」に書き換えないこと。環境変数の入れ忘れがそのまま穴になる。
- 🚨 **合言葉をブラウザへ渡さない。** 照合はサーバだけ。画面が持つのは
  「通した」ことの署名済みの証明だけで、そこにパスワードは入っていない。
  `VITE_ADMIN_PASSWORD` のような変数を**作らないこと**（バンドルに焼かれる）。
- **ステータスコードで行き先が変わる。** 401/403 = 運営ではない（トップへ帰す）、
  **422 = 合言葉が違う**（入力し直し）、423 = 合言葉が要る／切れた（入力へ）。
  合言葉の不一致に 401 を使うと、画面が「権限が無い」と誤解してトップへ飛ぶ。
- **証明の署名鍵は `ADMIN_PASSWORD` から作っている。** パスワードを変えれば
  発行済みの証明は全部無効になる（＝失効の手段）。ただし**再デプロイするまで実行時に反映されない**。
- 総当たり対策（5回で10分）は**プロセス内のカウンタ**。インスタンスが増えればその分試せるので、
  これは補助。本体は「運営のアカウントでログインしていないと POST まで届かない」ほう。

### 30-d. 確認したこと（2026-08-30）

- ✅ `npm run build` / `vercel build --prod` 通過。配信バンドルに `ADMIN_PASSWORD` の値は**入っていない**（grep 0件）。
  現行 ref（`melfyxfvhyknqhruytms`）・`pk_live_`・Turnstile サイトキーが焼き込まれていること、
  旧 ref が0件であることも確認済み（§2 の手順どおり。`vercel pull` は
  `VITE_STRIPE_PUBLISHABLE_KEY` と `VITE_TURNSTILE_SITE_KEY` を**空で書き出すので埋め戻した**）。
- ✅ **本番へデプロイ済み**（`dpl_ACJdc41Wstxh5ocn9MnNbaU6qu7y` / `aisekimatch.com` に alias 済み）。
- ✅ `.e2e-admin-gate.mjs`（未コミット）を**本番URLに対して**実行し **26項目すべて成功**:
  未ログイン（3つとも401）／一般ユーザー（gate・status・targets が403、正しい合言葉を送っても403で証明が出ない）／
  運営（gate 200・未解除は423・違う合言葉は422・正しい合言葉で証明が出て dm/* が200）／
  証明の悪用（署名の書き換え→423・他人が借りる→403・ログイン無しで証明だけ→401）。
  検証用の一般ユーザーは最後に削除済み。運営のセッションは
  `generate_link` → `verifyOtp` で取っている（メールは飛ばない・パスワードは使わない）。
